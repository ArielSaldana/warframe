...
#ifndef WARFRAME_DECODER_H
#define WARFRAME_DECODER_H
...
class Decoder {
public:
    Decoder(const std::vector<uint8_t> &array)
        : data(array.data()), size(array.size()), isInvalid(false), offset(0) {
        const auto version = read8();
        if (version != FORMAT_VERSION) {
            std::cout << "Error with format version, should exit" << std::endl;
            isInvalid = true;
        }
    }

    Decoder(const uint8_t *data_, size_t length_, bool skipVersion = false)
        : data(data_), size(length_), isInvalid(false), offset(0) {
        if (!skipVersion) {
            const auto version = read8();
            if (version != FORMAT_VERSION) {
                std::cout << "Error with format version, should exit" << std::endl;
    ...
    uint8_t read8() {
        if (offset + sizeof(uint8_t) > size) {
            std::cout << "Reading a byte passes the end of the buffer." << std::endl;
            return 0;
        }
...
//            THROW("Reading four bytes passes the end of the buffer.");
    ...
    int decodeSmallInteger() {
        return (read8());
    }

    int decodeInteger() {
        return ((int32_t) read32());
    }
...
//
    ...
    std::unordered_map<std::string, std::string> decodeMap() {
        ...
        for (uint32_t i = 0; i < length; ++i) {
            const auto key = unpack<std::string>();
            const auto value = unpack<std::string>();
            if (isInvalid) {
//                return Nan::Undefined();
            }
            map[key] = value;
//            Nan::Set(map, key, value);
    ...
    const char *readString(uint32_t length) {
        if (offset + length > size) {
            std::cout << "Reading sequence past the end of the buffer." << std::endl;
            return NULL;
        }
...
//    }
//
    std::string decodeBinaryAsString() {
...
//                return decodeCompressed();
            default:
                std::cout << "Unsupported erlang term type identifier found" << std::endl;
